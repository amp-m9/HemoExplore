...
export class SIOController {
    ...
    constructor(
        ...
        this.disabledObjects = new Array<SceneInteractiveObject>();
        ...
        canvas.addEventListener("click", (e) => {
            ...
            if(selected==undefined)
                return;
            selected.sio?.unHighlightFunction();
            selected.sio?.callbackFunction();
        });
    }
    ...
    update() {
        ...
        if (this.intersected.length > 0) {
            ...
            if (
                ...
                interactiveObject.highlightFunction == undefined
            )
                return;
        ...
        if (!(this.intersected.length > 0)) document.body.style.cursor = "auto";
    }

    getSelected() {
        let activeIndex = -1;
        const objectsSelected = this.intersected != undefined && this.intersected.length > 0;
        if(!objectsSelected)
            return;
        const object = this.interactiveObjects.find((interactiveObject, i) => {
            activeIndex = i;

            if (interactiveObject.object.isGroup) {
                const group = interactiveObject.object as THREE.Group;
                const matching = group.children.find(
                    (child) => child.id == this.intersected[0].object.id
                );
            ...
            return (
                objectsSelected &&
                interactiveObject.object.id == this.intersected[0].object.id
            );
        });
    ...
    disableObject(object: THREE.Object3D) {
        const sceneInteractiveObject = this.moveObjectById(object, false);

        if (sceneInteractiveObject == undefined) return;

        if (
            sceneInteractiveObject.highlighted &&
            sceneInteractiveObject.unHighlightFunction != undefined
        )
            sceneInteractiveObject.unHighlightFunction();
    }

    enableObject(object: THREE.Object3D) {
        const sceneInteractiveObject = this.moveObjectById(object, true);

        if (sceneInteractiveObject == undefined) return;

        if (
            sceneInteractiveObject.highlighted &&
            sceneInteractiveObject.highlightFunction != undefined
        )
            sceneInteractiveObject.highlightFunction();
    }

    moveObjectById(object: THREE.Object3D, enable: boolean) {
        let index = 0;
        const from = enable ? this.disabledObjects : this.interactiveObjects;
        const to = !enable ? this.disabledObjects : this.interactiveObjects;

        const sceneInteractiveObject = from.find((v, i) => {
            index = i;
            return v.object.id == object.id;
        });
        if (sceneInteractiveObject == undefined) return;

        from.splice(index, 1);
        to.push(sceneInteractiveObject);
